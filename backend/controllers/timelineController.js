...
const addNewEducation = async (req, res) => {
    try {
        const { startDate, endDate, collegeName, cgpa, description, link } = req.body;
...
const deleteEducation = async (req, res) => {
    try {
        const { id } = req.params;
        const educationDeleted = await educationModel.findByIdAndDelete({ _id: id });
        res.status(200).send({
...
const addNewExperience = async (req, res) => {
    try {
        const { startDate, endDate, jobPosition, CompanyName, description, link } = req.body;
...
const deleteExperience = async (req, res) => {
    try {
        const { id } = req.params;
        const experienceDeleted = await experienceModel.findByIdAndDelete({ _id: id });
        res.status(200).send({
            message: 'Deleted Successfully!!',
            experienceDeleted,
        });
